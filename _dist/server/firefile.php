...
class FireFileBase {
	...
	error_log("avant le token");
	    ...
	    error_log($result);
            $response = array();            
            if($result === true) {
	    	       error_log("if result = true");
                $saveResult = $this->saveChanges();
		error_log("apres save result");
                if($saveResult === true) {
			       error_log("save result est true");
                    $response["success"] = true;
                    $response["message"] = "file(s) successfully saved";
                }else{
			error_log("save result sest false");
                    $response["success"] = false;
                    $response["message"] = $saveResult;
                }
            }else{
		error_log("if result = false");
                $response["success"] = false;
	    ...
	    //flo : ceci est la source du problème : temps de render très long
            $this->render();
        }
	...
	error_log("==> dans le save changes");
    	// Get request data
	...
	error_log("l'adresse du fichier en question ::");
	error_log($file_abs_path);
    ...
    private function createDemoContent() {
        if(!file_exists("firefile.demo.css")) {
					      //TODO : restaure the test for this with local ressource
					      return true;
    ...
    private function saveFile($file, $contents, $force=false) {
        error_log("dans le save file");
    	if(file_exists($file) || $force) {
    		error_log("dans le file_exists");
		$handle = @fopen($file, 'w');
            	if($handle === false) {
			   error_log("pb avec le handl");
			   return false;
                }
		error_log("tout roule ma poule");
    		fwrite($handle, $contents);
    		fclose($handle);
    		return true;
    	}else{
		error_log("pas file exist");
    		return false;
    	}
    }
